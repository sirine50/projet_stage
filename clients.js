document.addEventListener("DOMContentLoaded", function () {
  ...
  function addContactRow(contact) {
    ...
    row
      .querySelector(".btn-remove-contact")
...
window.addEventListener("click", function(e){
      ...
      const contactsHtml =
        client.contacts && client.contacts.length > 0
          ? client.contacts
              .map(
                (c) =>
                  `<span class="contact-tag">${escapeHtml(c.nom || c.email || c.telephone || "Contact")}</span>`,
              )
              .join("")
  ...
  function updateCards() {
    ...
    const totalContacts = clients.reduce(
      (sum, c) => sum + (c.contacts ? c.contacts.length : 0),
      0,
    );

    cardClients.textContent = totalClients;
    cardActifs.textContent = totalClients;
  ...
  function applyFilter() {
    ...
    const filtered = clients.filter(function (c) {
      ...
      const paiementOK = paiement === "" || c.modePaiement === paiement;

      const conditionOK = condition === "" || c.conditionPaiement === condition;
